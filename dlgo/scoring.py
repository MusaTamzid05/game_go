...
class Territory:
    def __init__(self, territory_map):
        ...
        self.num_black_stone = 0
        self.num_white_stone = 0
        self.num_dame = 0
        self.dame_points = []

        for point, status in territory_map.items():
            if status == Player.black:
                self.num_black_stone += 1

            elif status == Player.white:
                self.num_white_stone += 1
...
def evaluate_territory(board):
    ...
    # Create stone for all the available position
    for r in range(1, board.num_rows + 1):
        for c in range(1, board.num_cols + 1):
            ...
            if stone is not None:
                ...
                if len(neighbors) == 1:
                    neigbore_stone = neighbors.pop()
                    stone_str = "b" if neigbore_stone == Player.black else "w"
...
def compute_game_result(game_state):
    ...
    return GameResult(territory.num_black_territory + territory.num_black_stone,
            territory.num_white_territory + territory.num_white_stone,
            komi = 7.5
            )
